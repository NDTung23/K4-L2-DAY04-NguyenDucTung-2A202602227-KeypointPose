...
4. Ảnh có OKS thấp nhất là `test_03.jpg`, người #2, với **OKS = 0.188** (thấp nhất trong toàn bộ danh sách "3 skeleton cần sửa trước"). Lỗi cụ thể: model bị **Đảo trái/phải** và **Trượt hẳn** ở hai khớp `left_elbow` và `right_wrist` — nghĩa là model đặt các khớp này sai hẳn vị trí, không chỉ lệch nhẹ. Trong trường hợp này, **gold (nhãn test có sẵn) đúng, model sai** — vì `dataset/labels/test` là nhãn tham chiếu chuẩn được lab cung cấp, còn lỗi đảo trái/phải và trượt hẳn là dấu hiệu rõ ràng của model dự đoán nhầm, không phải do nhãn gold có vấn đề.

*Lưu ý phương pháp:* để so sánh được, tôi phải tự quy đổi cột "visibility" của model (vốn là độ tin cậy 0–1) sang thang 0/1/2 bằng ngưỡng tạm (≥0.5 → v=2, ≥0.1 → v=1, còn lại → v=0). Ngoài ra, kết quả cho thấy model tạo ra **29 skeleton thừa** so với 13 người trong gold — có thể do ngưỡng `conf=0.25` khi predict còn thấp, khiến model phát hiện trùng lặp/nhiễu. Vì vậy con số OKS ở đây mang tính tham khảo, không hoàn toàn chính xác như cách chấm nhãn train (đã dùng đúng định dạng gốc, không qua quy đổi).
